...
class HttpAdapter {
  ...
  constructor() {
    ...
    this.client.interceptors.request.use((config) => {
      const token = localStorage.getItem("looptone-token");
      if (token) {
  ...
  async post<TResponse, TBody>(url: string, body: TBody, headers?: Record<string, string>): Promise<HttpResponse<TResponse>> {
    try {
      const response = await this.client.post<TResponse>(url, body, { headers });
